...
static void BitstreamReadFFNumber(AVCDecBitstream *stream, uint *result)
{
    uint value = 0;
    uint byte;
    do
    {
        BitstreamReadBits(stream, 8, &byte);
        value += byte;
    } while (byte == 0xff);
    *result = value;
}
        ...
        int *nal_size)
{
    ...
    if (video->sei.payload_size > MAX_SEI_PAYLOAD)
    {
        return AVCDEC_FAIL;
    }
    ...
    {
        uint bits;
        BitstreamReadBits(bitstream, 8, &bits);
        video->sei.payload[count] = bits;
    }
    return AVCDEC_SUCCESS;
}
/* ======================================================================== */
...
OSCL_EXPORT_REF AVCDec_Status PVAVCDecGetOutput(AVCHandle *avcHandle, int *indx, int *release, AVCFrameIO *output)
{
    ...
    if (video->sei.payload_type == AVC_SEI_USER_DATA_UNREGISTERED)
    {
        output->sei = &video->sei;
    }
    else
    {
        output->sei = 0;
